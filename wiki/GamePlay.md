...
和平语音系统是和平为绿洲玩法内置的功能，需要使用 [[api/class/detail/和平全局接口/社交系统/UGCVoiceManagerSystem|UGCVoiceManagerSystem]] 函数库动态启用，具体查看 [语音系统](https://developer.gp.qq.com/wikieditor/#/catalog/212)。

<br>

## Q2：如何修改玩家角色的形象

- 更换模型：技能编辑器提供了 [变身Buff模板](https://developer.gp.qq.com/wikieditor/#/catalog/20253)，可通过释放Buff来实现变身效果，但是仅支持变身为主角骨骼的模型；也支持在脚本中调用 [[api/class/detail/Others/UGCPlayerPawnSystem|ChangeAvatarMesh]] 动态切换模型，同样只支持主角骨骼
- 角色隐身：技能编辑器提供了 [隐身Buff模板](https://developer.gp.qq.com/wikieditor/#/catalog/20253?autoJump=%E9%9A%90%E8%BA%AB)，可通过释放Buff来实现隐身效果；也支持在脚本中调用 [[api/class/detail/Others/UGCPlayerPawnSystem|HideBoneByBoneName]] 方法隐藏指定的骨骼部位
...
新旧物编配备的函数库是独立的，对于用新物编创建的物品必须使用UGCBackpackSystemV2的函数 [[api/class/detail/和平全局接口/物品与背包/UGCBackpackSystemV2|AddItemV2]]，建议开发者遇到此类问题时，先检查创建物品所使用的物编版本和函数库是否一致。
...
- 新背包需在【玩法通用设置】设置是否显示背包按钮，也可以通过调用 [[api/class/detail/和平全局接口/物品与背包/UGCBackpackSystemV2|SetBackpackButtonVisible]] 函数控制背包按钮的动态显隐
