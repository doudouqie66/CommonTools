...
在编辑器顶部的 ``玩法通用设置`` 中可以设置是否显示背包按钮，也可以通过 [[api/class/detail/和平全局接口/物品与背包/UGCBackpackSystemV2|``SetBackpackButtonVisible``]] 函数控制背包按钮的动态显隐。
...
如果需要自定义解锁逻辑，可重写 ``BP_BackpackUIComponentV2`` 中的 [[api/class/detail/Others/BackpackUIComponent|``ClickLockBackpackItem``]] 函数，通过接口 [[api/class/detail/和平全局接口/物品与背包/UGCBackpackSystemV2|``AddCellCapacity``]] 为背包解锁格子，解锁增加的容量是持久化的。
...
与之相对的，[[api/class/detail/和平全局接口/物品与背包/UGCBackpackSystemV2|``AddMaxCellCapacity``]] 接口增加的容量不会持久化，与背包扩容不同。
...
可通过接口 [[api/class/detail/和平全局接口/物品与背包/UGCBackpackSystemV2|``OpenBackpackPanelStyle``]] 打开背包。
...
仓库默认为关闭状态，需要在脚本中适当的时机调用 [[api/class/detail/和平全局接口/物品与背包/UGCBackpackSystemV2|``OpenBackPackPanel``]] 以启用。
...
|函数库/类名|函数类型|函数功能范围|
|-|-|-|
|[[api/class/detail/和平全局接口/物品与背包/UGCBackpackSystemV2|UGCBackpackSystemV2]]|对象函数|V2背包对物品和装备的操作、内置仓库、容量限制以及货币的查询|
|[[api/class/detail/和平全局接口/物品与背包/UGCItemSystemV2|UGCItemSystemV2]]|对象函数|读取和查询物品信息|
