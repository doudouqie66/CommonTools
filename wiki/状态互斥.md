...
玩法中可以通过进入状态与退出状态来表示角色特定状态的切换，[[api/class/detail/和平全局接口/技能系统/UGCPersistEffectSystem|UGCPersistEffectSystem]] 库提供了 [[api/class/detail/和平全局接口/技能系统/UGCPersistEffectSystem|``EnterDynamicState``]] 和 [[api/class/detail/和平全局接口/技能系统/UGCPersistEffectSystem|``LeaveDynamicState``]] 两个函数分别用于实现状态的进入与退出。同一种状态允许多次进入，且每次进入状态都会累积计数，开发者可以通过 [[api/class/detail/和平全局接口/技能系统/UGCPersistEffectSystem|``HasDynamicState``]] 判断当前角色是否存在该状态。
...
脚本中可以通过是否能进入某个Tag标识的状态，来判断该状态是否已被禁用，如果被禁用则不允许进入该状态，开发者调用 [[api/class/detail/和平全局接口/技能系统/UGCPersistEffectSystem|``AllowDynamicState``]] 可获取判定结果。
...
脚本中调用 [[api/class/detail/和平全局接口/技能系统/UGCPersistEffectSystem|``InterruptDynamicState``]] 和 [[api/class/detail/和平全局接口/技能系统/UGCPersistEffectSystem|``SetDynamicStateDisabled``]] 可实现主动打断/禁用其他状态的效果。
