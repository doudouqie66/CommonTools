...
玩法中经常根据特定对象或者伤害源产生的角色/怪物伤害进行二次修改，例如放大对指定部位的伤害、增强特定玩家/物品造成的伤害等等，现有功能体系下实现 [[api/class/detail/和平类事件/主角类（PlayerPawn）/ASTExtraBaseCharacter|伤害覆写]] 的逻辑分散在各个类脚本中难以管理。
...
通过 [[api/class/detail/和平全局接口/角色系统/UGCAttributeSystem|``UGCAttributeSystem``]] 库提供的函数可以从 ``Context`` 中获取本次伤害的具体情境信息，例如 [[api/class/detail/和平全局接口/角色系统/UGCAttributeSystem|``GetVictimFromContext``]] 获取受害者对象、[[api/class/detail/和平全局接口/角色系统/UGCAttributeSystem|``GetSourceMagnitudeFromContext``]] 获取原始伤害值等，方便开发者基于特定的条件实现伤害逻辑。

此外部分系统功能支持触发伤害的行为配置，例如技能编辑器提供的 [技能Task-造成伤害](https://developer.gp.qq.com/wikieditor/#/catalog/20094?autoJump=%E6%8A%80%E8%83%BDTask-%E9%80%A0%E6%88%90%E4%BC%A4%E5%AE%B3)，Buff编辑器提供的 [造成伤害](https://developer.gp.qq.com/wikieditor/#/catalog/20117?autoJump=%E9%80%A0%E6%88%90%E4%BC%A4%E5%AE%B3) 动作，UGCAttributeSystem库也提供了相应的获取方式 [[api/class/detail/和平全局接口/角色系统/UGCAttributeSystem|``GetDamageTypeFromContext``]] 和 [[api/class/detail/和平全局接口/角色系统/UGCAttributeSystem|``GetDamageTagsFromContext``]]。
...
例如，监听了 [[api/class/detail/和平全局接口/工具库/UGCGenericMessageSystem|``UGCGenericMessageSystem.Messages.UGC.PlayerPawn.PostTakeDamage``]] 事件，触发伤害时判断当次伤害产生暴击，为 ``ExtraResult`` 添加暴击的Tag：
...
当角色受到伤害时，在回调伤害公式前后分别触发两个事件：[[api/class/detail/和平全局接口/工具库/UGCGenericMessageSystem|``UGCGenericMessageSystem.Messages.UGC.PlayerPawn.PreTakeDamage``]] 和 [[api/class/detail/和平全局接口/工具库/UGCGenericMessageSystem|``UGCGenericMessageSystem.Messages.UGC.PlayerPawn.PostTakeDamage``]]，开发者可以监听该事件实现自定义处理逻辑。
