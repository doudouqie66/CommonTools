...
玩法逻辑中只需调用 [[api/class/detail/和平全局接口/UI 界面/UGCWidgetManagerSystem|``UGCWidgetManagerSystem``]] 的API加载WidgetLayout蓝图类，UI系统会重载或覆盖和平的原生界面，从而按WidgetLayout的设置正确显示各控件的状态。
...
蓝图配置控件UISlot的方式目前仅支持全局层级锚点类型，如果需要对特定和平控件挂载自定义的控件，可以调用 [[api/class/detail/和平全局接口/UI 界面/UGCWidgetManagerSystem|``AddToSlot``]] 来实现同类效果。

---

### 加载与卸载WidgetLayout

创建与设置完WidgetLayout蓝图后，开发者可以在需要的场景和合适的时机下（确保和平主UI已完成初始化及加载，建议PlayerController/Pawn的Beginplay或之后），于客户端调用 [[api/class/detail/和平全局接口/UI 界面/UGCWidgetManagerSystem|``SetWidgetLayout``]] 完成WidgetLayout的加载或卸载（传参为“Default”为卸载WidgetLayout效果）。
