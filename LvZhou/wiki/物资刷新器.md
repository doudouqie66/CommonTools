...
其中 `CustomID` 为自定义参数列表，即在 `物资刷新点Actor` 上配置的自定义模式参数，该事件需使用 [[api/class/detail/Others/AUGCItemSpawner|**SpawnItem**]] 方法生成具体的物资数据，合并成 `Table<Table<KeyType, ValueType>>` 类型数据返回。
...
>注：物资刷新器相关API可查询文档：[[api/class/detail/Others/AUGCItemSpawner|物资刷新器]]
...
如上所示， [[api/class/detail/和平全局接口/物品与背包/AUGCItemSpawnerManager|**StartSpawnerManager**]] 为 `物资刷新管理器` 自身开启物资刷新的方法。

>注：物资刷新管理器相关API可查询文档：[[api/class/detail/和平全局接口/物品与背包/AUGCItemSpawnerManager|物资刷新管理器]]
