...
- Dynamic Affector Update Mode：导航网格的更新模式
	...
	- Manual Trigger：手动更新，需要自行调用 [[api/class/detail/Others/UGCNavigationSystem|``AsyncIncrementalBuild``]] 触发更新
...
[[api/class/detail/Others/UGCNavigationSystem|``AsyncBuildNavmesh``]] 会对场景中所有寻路体积覆盖的区域进行一次性导航网格数据更新，在服务器端调用此接口即可进行导航网格的动态刷新。
...
要想触发导航网格数据的局部更新，需要先调用 [[api/class/detail/Others/UGCNavigationSystem|``AddDynamicNavAffect``]] 标记待更新的区域，该区域将被缓存直到被更新模式触发为止：

- 手动更新模式：在合适的时机下脚本中调用 [[api/class/detail/Others/UGCNavigationSystem|``AsyncIncrementalBuild``]] 主动触发更新
- 自动更新模式：系统将在 ``Dynamic Affector Update Interval`` 间隔时间后自动调用 [[api/class/detail/Others/UGCNavigationSystem|``AsyncIncrementalBuild``]]
...
在局部更新时，先标记待更新区域,为了更好地演示，在这里对于标记的区域进行绘制以便观察。点击按钮"局部更新区域标记"调用接口 [[api/class/detail/Others/UGCNavigationSystem|``AddDynamicNavAffect``]] 标记待更新的区域，即可发现红色线框区域为标记区域。
...
标记完成后，如果选择手动更新模式，则需要手动调用更新，点击按钮"局部更新手动调用"手动调用接口 [[api/class/detail/Others/UGCNavigationSystem|``AsyncIncrementalBuild``]] 触发更新。更新完成后即可发现标记区域导航网格数据已被更新。
...
[[api/class/detail/Others/UGCNavigationSystem|``GetNavigationGenerationFinishedDelegate``]] 可以获取到更新结束的委托，进而绑定回调函数以执行特定处理逻辑。
