...
通过 [[api/class/detail/和平全局接口/角色系统/UGCAttributeSystem|``GetDamagePositionTypeFromContext``]] 接口可获取受击部位的信息，其参数是伤害上下文数据，返回值类型为枚举 [[api/cppenum/detail/EAvatarDamagePosition|EAvatarDamagePosition]] 。
...
开发者可以根据自身需求，从服务端调用 [[api/class/detail/和平全局接口/基础功能/UGCGameSystem|UGCGameSystem]] 的伤害函数来实现伤害的给予，以下是这些接口的详细说明：

|伤害函数|作用说明|受击判定说明|
|:-:|:-:|:-:|
|[[api/class/detail/和平全局接口/基础功能/UGCGameSystem|ApplyRadialDamageWhiteList]]|对指定列表内的 Actor 造成爆炸伤害|统一判定为 ``BigBody`` |
|[[api/class/detail/和平全局接口/基础功能/UGCGameSystem|ApplyRadialDamage]]|造成爆炸伤害|统一判定为 ``BigBody`` |
|[[api/class/detail/和平全局接口/基础功能/UGCGameSystem|ApplyDamage]]|造成伤害|统一判定为 ``BigBody`` |
|[[api/class/detail/和平全局接口/基础功能/UGCGameSystem|ApplyPointDamage]]|造成点伤害|可精确判定受击部位|
|[[api/class/detail/和平全局接口/基础功能/UGCGameSystem|ApplyAvatarPositionDamage]]|造成部位伤害|可精确判定受击部位|
