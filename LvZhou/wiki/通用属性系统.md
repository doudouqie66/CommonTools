...
[[api/class/detail/和平全局接口/角色系统/UGCAttributeSystem|``UGCAttributeSystem``]] 提供了完善的操作属性的接口与委托，包括查询、修改以及监听属性数值变化的委托等，开发者可以在脚本中调用相关函数对属性值进行动态调整，使用属性前需要主动引用该脚本文件，并通过属性名映射表的key来访问对应属性。
