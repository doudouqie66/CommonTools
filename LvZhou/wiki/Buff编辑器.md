...
> 开发者也可以利用 [[api/class/detail/Others/UPersistEffectBuff|``UPersistEffectBuff``]] 提供的查询类API自行实现Buff的UI外显效果
...
[[api/class/detail/和平全局接口/技能系统/UGCPersistEffectSystem|UGCPersistEffectSystem]] 库提供了添加和移除Buff的API，例如 [[api/class/detail/和平全局接口/技能系统/UGCPersistEffectSystem|``AddBuffByClass``]]) 可以给目标对象添加Buff。
