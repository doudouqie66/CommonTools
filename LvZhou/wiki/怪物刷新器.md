...
刷新器提供了 [[api/class/detail/Others/AUGCMobSpawner|``CustomSpawnMob``]] 函数用于覆写生成怪物的逻辑，该函数的入参是配置在 ``自定义参数`` 中的参数组，开发者可以通过该属性透传任意参数用于实现特定逻辑，在函数体中调用刷新器提供的 [[api/class/detail/Others/AUGCMobSpawner|``SpawnMob``]] 函数进行怪物的生成。
...
|属性名|属性说明|
|-|-|
|刷怪管理器启动方式|支持关卡加载、事件触发和手动调用三种方式<br>- 关卡加载：当管理器所属的关卡加载后自动启动刷怪流程<br>- 事件触发：当接收到 ``事件名`` 属性中填写的指定GMP事件时触发启动刷怪流程<br>- 手动调用：管理器无法自动启动，需要在脚本中调用 [[api/class/detail/和平全局接口/物品与背包/AUGCItemSpawnerManager|``StartSpawnerManager``]] 触发|
...
|函数库/类名|函数类型|函数功能范围|
|-|-|-|
|[[api/class/detail/Others/AUGCMobSpawner|AUGCMobSpawner]]|对象函数|修改刷新点配置、生成怪物及相应的刷新事件|
|[[api/class/detail/和平全局接口/怪物系统/AUGCMobSpawnerManager|AUGCMobSpawnerManager]]|对象函数|刷怪管理器的启动、暂停、重置等运行时管理及配置修改和刷新事件|