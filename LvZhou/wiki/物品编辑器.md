...
- 空白物品：默认不具备特性属性和功能，可用于创建玩法中类似货币、兑换物等虚拟物品，也能够结合 [[api/class/detail/和平全局接口/物品与背包/UGCBackpackSystemV2|背包系统API]] 实现具有特定功能的物品
...
基于空白物品/消耗品模板创建的物品，需要结合 [[api/class/detail/Others/UBackpackComponentV2|``OnUseItemV2``]]、[[api/class/detail/Others/UBackpackComponentV2|``OnMergeItemV2``]] 等物品交互的函数实现相关具体功能效果，以空白消耗品模板为例，实现一个使用物品效果为将使用者变身成光子鸡的物品。
...
- 对于和平经典背包调用 [[api/class/detail/和平全局接口/物品与背包/UGCBackPackSystem|``AddItem``]]
- 如果玩法 [启用了新背包系统](https://developer.gp.qq.com/wikieditor/?timeStamp=1725589224096#/catalog/20104?autoJump=%E5%90%AF%E7%94%A8%E8%83%8C%E5%8C%85%E7%B3%BB%E7%BB%9F) 可以调用 [[api/class/detail/和平全局接口/物品与背包/UGCBackpackSystemV2|``AddItemV2``]] 或者 [[api/class/detail/和平全局接口/物品与背包/UGCBackpackSystemV2|``AddItemByDefineIDV2``]]
...
开发者可以使用 [背包系统](https://developer.gp.qq.com/wikieditor/?timeStamp=1725589224096#/catalog/20104) 提供的丰富API扩展和实现物品的交互逻辑，例如通过 [[api/class/detail/和平全局接口/物品与背包/UGCBackpackSystemV2|``ItemCanEquipToSlot``]] 和 [[api/class/detail/和平全局接口/物品与背包/UGCBackpackSystemV2|``EquipItemV2``]] 扩展物品的装配规则和逻辑；通过 [[api/class/detail/Others/UBackpackComponentV2|``CanUseItemV2``]] 和 [[api/class/detail/和平全局接口/物品与背包/UGCBackpackSystemV2|``UseItemV2``]] 扩展物品的使用规则和逻辑等。
