...
所有进入对局的玩家都会拥有各自的队伍编号，即使玩家以单人进入对局，系统也会为其分配一个默认的队伍编号；如果涉及二次分队，例如非对称类型的玩法，则需要调用 [[api/class/detail/和平全局接口/社交系统/UGCTeamSystem|``ChangePlayerTeamID``]] 进行动态调整。
...
新的阵营系统提供了更灵活的敌对关系配置，同时配合新的 [怪物系统](https://developer.gp.qq.com/wikieditor/#/catalog/20144) 同步支持怪物的阵营分配，开发者仅需通过蓝图参数设置即可实现阵营的分配，也支持通过 [[api/class/detail/和平全局接口/社交系统/UGCCampSystem|UGCCampSystem]] 提供的接口动态调整阵营。
...
如果希望在玩法进程中动态修改玩家的阵营，可以调用 [[api/class/detail/和平全局接口/社交系统/UGCCampSystem|`SetCampForTeam`]] 进行修改，需要注意的是单个玩家无法单独设置阵营，仅能通过队伍所属阵营间接修改玩家阵营。
...
如果希望在玩法进程中动态修改怪物的阵营，可以调用 [[api/class/detail/和平全局接口/社交系统/UGCCampSystem|`SetCampForActor`]] 进行修改。
...
|函数库/类名|函数类型|函数功能范围|
|-|-|-|
|[[api/class/detail/和平全局接口/社交系统/UGCTeamSystem|UGCTeamSystem]]|静态函数|动态设置、获取玩家队伍ID，查询队友、队长等|
|[[api/class/detail/和平全局接口/社交系统/UGCCampSystem|UGCCampSystem]]|静态函数|动态修改、获取阵营关系|