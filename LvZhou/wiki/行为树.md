...
开发者可使用 [[api/class/detail/和平全局接口/怪物系统/UGCGenericCharacterSystem|```UGCGenericCharacterSystem```]] 库动态调度行为树的启用与停止，以更加方便的控制怪物行为的切换和动态调整，如下使用在场景中添加按钮的方法展示。
...
```UBlackboardComponent``` 是怪物蓝图的一个组件，怪物类可使用 [[api/class/detail/Others/AUGCGenericCharacter|```GetBlackBoardComponent```]] 方法获取实例对象，进而可使用黑板组件获取或修改变量的值，具体API可查看文档：[[api/class/detail/Others/UBlackboardComponent|```UBlackboardComponent```]] 。
